...
class Vocab:
    ...
    """
    def __init__(self,vocab_size:int=16000,pad_token:str="[PAD]",
        ...
        self.speical_tokens = [pad_token,unk_token,bos_token,eos_token]
        self.freqs = Counter()
        self.vocab = OrderedDict()

        #使用特殊字符初始化字典
        for s in self.speical_tokens:
            self.vocab[s] = len(self.vocab)

    def build(self,corpus,tokenizer,max_setence_length=100000):
        with open(corpus,"r",encoding="utf8") as reader:
            for i,line in enumerate(reader.readlines()):
                if len(line)>max_setence_length:
                    line = line[:max_setence_length]

                tokens = tokenizer.tokenize(line.strip())

                self.freqs.update(tokens)

        for token,freq in self.freqs.values.most_common(self.vocab_size-len(self.speical_tokens)):
            self.vocab[token] = len(self.vocab)

    def save(self,path,postfix = ".vocab"):
        with open(path+postfix,"w",encoding="utf8") as writer:
